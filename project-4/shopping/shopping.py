    ...
    with open(filename, newline='') as f:
        ...
        for row in reader:
            evidence.append([
                ...
                month_converter(row[10]),  # Month, an index from 0 (January) to 11 (December)
                ...
                int(1) if row[15] == "Returning_Visitor" else int(0),  # VisitorType, an integer 0 (not returning) or 1 (returning)
                int(1) if row[16] == "TRUE" else int(0)  # Weekend, an integer 0 (if false) or 1 (if true)
            ])
